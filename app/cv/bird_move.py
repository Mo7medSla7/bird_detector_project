...
class cls_motion(object):
    ...
    def process_frames(self,path):
        ...
        for fileName in frames:
            ...
            dict_frame_time[frame_second]=os.path.splitext(fileName)[0].replace(frame_second,'')
    ...
    def get_pose(self, fileName_withoutTime_withoutExt):
        ...
        json_fileName_withoutTime_withoutExt=fileName_withoutTime_withoutExt.replace('.',':').replace('_',',').lower()
        #('%f+i%f' % (r.real, r.imag))
        json_fileName_withoutTime_withoutExt=('{%s}' % json_fileName_withoutTime_withoutExt )
        birds=[]
