...
from app.cv.Preidct import Predictor
from queue import Queue

from app.cv.myTime import Time
...
class ClsMotion(object):
    ...
    def process_frames(self , sheetPath):
        # self.frames_path = frames_path
        ...
        self.get_motion_per_time(dict_frame_time,sheetPath)

    def process_vedio(self, vedio_path):
        q = Queue(3000)
        model_path = 0
        predictor = Predictor(model_path,vedio_path , q )
        predictor.start()
        dict_frame_time = {}
        bird_pose , frame_number = q.get( timeout = 30 )

        while bird_pose is not None :
                    
            frame_time = float(frame_number)
            
            dict_frame_time[frame_time] = bird_pose

        self.get_motion_per_time(dict_frame_time)

    def get_motion_per_time(self, dict_frame_time,sheetPath):
        ...
        self.data_frame(dict_frame_time, tbl_movement,sheetPath)
    ...
    def data_frame(self, file_name, change,sheetPath):
        head = Details.head.value
        leg = Details.leg.value
        wing = Details.wing.value
        tail = Details.tail.value
        ...
        export_details.to_excel(
            excel_writer=sheetPath, sheet_name="Details")
        # export_ready_dataframe.to_excel(excel_writer=self.sheetPath, sheet_name="totals")


if __name__ == '__main__':
    # sheet path, frames path
    ob = ClsMotion(r"D:\poject software\excelsheet\s1000.xlsx")
    ob.process_frames(
        r"D:\poject software\frames\chronic egret 3rd black tool making TS 13 h")
